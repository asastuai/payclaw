 ...
 *
 * If any of these tests fail, PayClaw SDK's canonical-hash construction
 * has drifted from the spec. The cross-implementation verification chain
 * across BaseOracle, TrustLayer, Vigil, PayClaw, and proof-of-context-impl
 * depends on every implementation producing these exact hashes.
 */

import { test } from 'node:test';
import assert from 'node:assert/strict';
import { canonicalHash } from '../src/poc';
...
for (const v of VECTORS) {
  test(`vector ${v.id} produces the expected SHA-256 hash`, () => {
    ...
    assert.equal(
      ...
      `PayClaw SDK canonical-hash drift detected on ${v.id}.\n` +
